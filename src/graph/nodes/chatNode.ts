...
export function createChatNode(llmClient: OpenRouterService, preferenceService: PreferenceService) {
  return async (state: GraphState, runtime?: Runtime): Promise<Partial<GraphState>> => {
    const userId = String(runtime?.context?.userId || state.userId || 'unknown');
    const userContext = state.userContext ?? (await preferenceService.getBasicInfo(userId));
